...
public class DiskStateManager {
    ...
    private func prepareHDDRestore(_ hddState: DiskMountState.HDDState) -> PreparedHDDRestore? {
        ...
        do {
            // Validate and only fetch file size here; avoid loading full image into memory
            let attributes = try FileManager.default.attributesOfItem(atPath: restoreURL.path)
            guard let fileSize = attributes[.size] as? Int64 else {
                warningLog("HDD restore skipped: could not determine file size: \(hddState.filePath)", category: .fileSystem)
                return nil
            }
            ...
            // Enforce reasonable maximum to avoid memory exhaustion
            if fileSize > X68Security.maxDiskImageSize {
                errorLog("HDD restore skipped: image too large (\(fileSize) bytes): \(hddState.filePath)", category: .fileSystem)
                return nil
            }
            ...
            errorLog("Failed to stat HDD file for restore: \(error)", category: .fileSystem)
            return nil
        }
    }
    ...
    private func mountPreparedHDDRestore(_ prepared: PreparedHDDRestore, hddState: DiskMountState.HDDState) -> Bool {
        ...
        // Allocate buffer based on known file size and stream file into it to avoid double-copying large Data
        ...
        // Stream file contents directly into emulator buffer on a background queue
        let fileURL = URL(fileURLWithPath: prepared.filePath)
        do {
            ...
            while bytesCopied < prepared.fileSize {
                let toRead = Int(min(Int64(chunkSize), prepared.fileSize - bytesCopied))
            ...
            if bytesCopied != prepared.fileSize {
                errorLog("Streamed bytes mismatch for HDD restore: expected \(prepared.fileSize), got \(bytesCopied)", category: .fileSystem)
                return false
            }
        } catch {
            errorLog("Failed to stream HDD file into buffer: \(error)", category: .fileSystem)
            return false
        }
...
class FileSystem {
    ...
    private func performFileLoad(url: URL) {
        do {
            // Determine file size first to decide strategy
            let attributes = try FileManager.default.attributesOfItem(atPath: url.path)
            guard let fileSize = attributes[.size] as? Int64 else {
                errorLog("Could not determine file size for: \(url.path)", category: .fileSystem)
                return
            }

            // If file is small, load into memory; otherwise stream
            if fileSize <= X68Security.maxFileSize {
                let imageData: Data = try X68Security.safeLoadFile(url)
                ...
                // Stream large file into buffer on background queue
                DispatchQueue.global(qos: .userInitiated).async {
                    do {
                        // Enforce absolute maximum
                        guard fileSize <= X68Security.maxDiskImageSize else {
                            errorLog("File too large to load: \(fileSize) bytes", category: .fileSystem)
                            return
                        }

                        let extname = url.pathExtension.removingPercentEncoding ?? ""

                        // Determine target pointer and drive
                        let targetDrive = (extname.lowercased() == "hdf") ? 4 : ((url.path.contains(" B.") || url.path.contains("_B.")) ? 1 : 0)

                        guard let p = X68000_GetDiskImageBufferPointer(targetDrive, Int(fileSize)) else {
                            errorLog("Failed to get buffer pointer for streaming file: \(url.path)", category: .fileSystem)
                            return
                        }
                        ...
                        if bytesCopied != fileSize {
                            errorLog("Streamed bytes mismatch: expected \(fileSize), got \(bytesCopied)", category: .fileSystem)
                            return
                        }
                        ...
                        DispatchQueue.main.async {
                            if extname.lowercased() == "hdf" {
                                X68000_LoadHDD(url.path)
                                infoLog("HDD loaded successfully - streamed", category: .fileSystem)
                            } else {
                                let drive = (url.path.contains(" B.") || url.path.contains("_B.")) ? 1 : 0
                                X68000_LoadFDD(drive, url.path)
                                infoLog("FDD loaded successfully (drive \(drive)) - streamed", category: .fileSystem)
                            }
                        }
                    } catch {
                        errorLog("Error streaming file data", error: error, category: .fileSystem)
                    }
                }
            }
        } catch let error as NSError {
    ...
    private func loadCompanionDiskWithFallback(companionUrl: URL, completion: @escaping () -> Void) {
        ...
        do {
            let attributes = try FileManager.default.attributesOfItem(atPath: companionUrl.path)
            guard let fileSize = attributes[.size] as? Int64, fileSize > 0 else {
                infoLog("Companion disk empty or inaccessible: \(companionUrl.lastPathComponent)", category: .fileSystem)
                completion()
                return
            }
            guard fileSize <= X68Security.maxDiskImageSize else {
                infoLog("Companion disk too large: \(fileSize) bytes", category: .fileSystem)
                completion()
                return
            }
            ...
            // Stream into emulator buffer on background queue
            DispatchQueue.global(qos: .userInitiated).async {
                do {
                    guard let p = X68000_GetDiskImageBufferPointer(drive, Int(fileSize)) else {
                        errorLog("Failed to get buffer for companion disk", category: .fileSystem)
                        DispatchQueue.main.async { completion() }
                        return
                    }

                    let fh = try FileHandle(forReadingFrom: companionUrl)
                    defer { try? fh.close() }

                    var bytesCopied: Int64 = 0
                    let chunkSize = 256 * 1024 // 256KB for floppies
                    while bytesCopied < fileSize {
                        let toRead = Int(min(Int64(chunkSize), fileSize - bytesCopied))
                        let chunk = try fh.read(upToCount: toRead) ?? Data()
                        if chunk.isEmpty { break }
                    ...
                    if bytesCopied != fileSize {
                        errorLog("Companion disk streamed bytes mismatch: expected \(fileSize), got \(bytesCopied)", category: .fileSystem)
                        DispatchQueue.main.async { completion() }
                        return
                    }
    ...
    private func loadDiskFileWithData(_ url: URL, completion: @escaping (Bool) -> Void) {
        ...
        do {
            ...
            let attributes = try FileManager.default.attributesOfItem(atPath: url.path)
            guard let fileSize = attributes[.size] as? Int64, fileSize > 0 else {
                errorLog("File appears empty: \(url.path)", category: .fileSystem)
            ...
            // Stream on background queue and perform mount on main
            DispatchQueue.global(qos: .userInitiated).async {
                do {
                    guard let p = X68000_GetDiskImageBufferPointer(drive, Int(fileSize)) else {
                        errorLog("Failed to get buffer pointer for streaming file: \(url.path)", category: .fileSystem)
                    ...
                    DispatchQueue.main.async {
                        ...
                        if extname == "hdf" {
                            ...
                            X68000_LoadFDD(drive, url.path)
                            infoLog("Success: FDD loaded: \(url.lastPathComponent)", category: .fileSystem)
                            completion(true)
                        }
                    }
                } catch {
                    errorLog("Error streaming disk file: \(error)", error: error, category: .fileSystem)
                    DispatchQueue.main.async { completion(false) }
                }
            }
    ...
    func loadSRAM()
    {
        ...
        func loadSRAMData(from filename: String) -> Data? {
            ...
            do {
                let size = try X68Security.getFileSize(url)
                guard size == 0x4000 else {
                    errorLog("Security: Invalid \(filename) size: \(size), expected 0x4000", category: .fileSystem)
                    return nil
                }
                return try X68Security.secureFileAccess(url) {
                    let fh = try FileHandle(forReadingFrom: url)
                    defer { try? fh.close() }
                    var result = Data()
                    result.reserveCapacity(0x4000)
    ...
    func loadCGROM() -> Bool
    {
        ...
        do {
            let data: Data = try X68Security.safeLoadFile(url)
    ...
    func loadIPLROM() -> Result<Void, ROMLoadError>
    {
        ...
        if let sasiRomURL = findFileInDocuments("IPLROM0.DAT") {
            do {
                let sasiData: Data = try X68Security.safeLoadFile(sasiRomURL)
        ...
        if UserDefaults.standard.integer(forKey: "StorageBusMode") == 1 {
            ...
            do {
                let scsiData: Data = try X68Security.safeLoadFile(scsiRomURL)
    ...
    func loadFDDToDrive(_ url: URL, drive: Int) {
        ...
        do {
            let attributes = try FileManager.default.attributesOfItem(atPath: url.path)
            guard let fileSize = attributes[.size] as? Int64, fileSize > 0 && fileSize <= 2 * 1024 * 1024 else {
                errorLog("Security: Disk image file invalid size for FDD: \(url.path)", category: .fileSystem)
                return
            }

            guard let p = X68000_GetDiskImageBufferPointer(drive, Int(fileSize)) else {
                errorLog("Failed to get disk buffer pointer for FDD", category: .fileSystem)
                return
            }
            ...
            if bytesCopied != fileSize {
                errorLog("Failed to stream complete FDD image: expected \(fileSize), got \(bytesCopied)", category: .fileSystem)
                return
            }
