...
class GameScene: SKScene {
    ...
    func loadHDD(url: URL) {
        ...
        if allowedExtensions.contains(extname) {
            ...
            if extname == "hds" {
                // Stream SCSI HDD image into emulator buffer on a background queue
                DispatchQueue.global(qos: .userInitiated).async {
                    do {
                        let attributes = try FileManager.default.attributesOfItem(atPath: url.path)
                        guard let fileSize = attributes[.size] as? Int64 else {
                            errorLog("SCSI HDD: could not determine file size: \(url.path)", category: .fileSystem)
                            return
                        }
                        guard fileSize > 0 && fileSize <= X68Security.maxDiskImageSize else {
                            errorLog("SCSI HDD file too large or zero: \(fileSize) bytes", category: .fileSystem)
                            return
                        }
                        ...
                        if bytesCopied != fileSize {
                            errorLog("Streamed bytes mismatch for SCSI image: expected \(fileSize), got \(bytesCopied)", category: .fileSystem)
                            return
                        }
            ...
            do {
                // Do not read the entire HDD into memory on the main thread.
                let attributes = try FileManager.default.attributesOfItem(atPath: url.path)
                guard let fileSize = attributes[.size] as? Int64 else {
                    errorLog("HDD file: could not determine size: \(url.path)", category: .fileSystem)
                    return
                }
                ...
                guard fileSize <= X68Security.maxDiskImageSize else {
                    errorLog("HDD file too large: \(fileSize) bytes", category: .fileSystem)
                    return
                }

                // Stream file into emulator buffer on background queue
                DispatchQueue.global(qos: .userInitiated).async {
                    do {
                        // Request buffer pointer
                        guard let p = X68000_GetDiskImageBufferPointer(4, Int(fileSize)) else {
                        ...
                        if bytesCopied != fileSize {
                            errorLog("Streamed bytes mismatch for HDD image: expected \(fileSize), got \(bytesCopied)", category: .fileSystem)
                            return
                        }
                        ...
                        errorLog("Error streaming HDD file", error: error, category: .fileSystem)
                    }
                }
            } catch {
    ...
    private func restoreScsiFromDefaultsIfNeeded() -> Bool {
        ...
        do {
            ...
            guard fileSize <= X68Security.maxDiskImageSize else {
                warningLog("SCSI restore skipped: image too large (\(fileSize) bytes)", category: .fileSystem)
                return false
            }
            ...
            if bytesCopied != fileSize {
                warningLog("SCSI restore streamed bytes mismatch: expected \(fileSize), got \(bytesCopied)", category: .fileSystem)
                return false
            }
