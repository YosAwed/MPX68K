...
void FASTCALL BG_SetDoubleBuffer(int enable)
{
	...
	if (enable) {
		// ダブルバッファモード: 初期は同一バッファを指して即座に表示されるようにする
		BG_LineBuf_Active = BG_LineBuf_Back;
		BG_PriBuf_Active = BG_PriBuf_Back;
		BG_LineBuf_Draw = BG_LineBuf_Back;
		BG_PriBuf_Draw = BG_PriBuf_Back;
	} else {
