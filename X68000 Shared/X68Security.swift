...
class X68Security {
    ...
    /// 最大ファイルサイズ読み込み（アプリ内の一般的な操作向け、既定: 100MB）
    static let maxFileSize: Int64 = 100 * 1024 * 1024

    /// 最大ディスクイメージサイズ（HDD/SCSI 用、2GB）
    ...
    static func calculateChecksum(_ url: URL) throws -> String {
        return try secureFileAccess(url) {
            ...
            while true {
                let chunk = try fh.read(upToCount: chunkSize)
    ...
    static func safeLoadFile(_ url: URL) throws -> Data {
        ...
        // Enforce maxFileSize to avoid unexpectedly large memory allocations
        let size = try getFileSize(url)
        guard size <= maxFileSize else {
            let sizeMB = Int(size / (1024 * 1024))
            throw X68MacError.fileTooLarge(url.lastPathComponent, sizeMB)
        }

        // セキュリティスコープ付きでファイル読み込み（small files only）
        return try secureFileAccess(url) {
            let fh = try FileHandle(forReadingFrom: url)
            defer { try? fh.close() }
