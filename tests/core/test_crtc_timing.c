 ...
 *  2. The model agrees with the renderer globals that crtc.c computes today
...
static void test_timing_768x512(void)
{
    ...
    CHECK_EQ(t.v_step, 2, "768x512: normal v_step 2");
...
static void test_1024line_interlace(void)
{
    ...
    /* HF=1 with VRES=2 is a 1024-line interlace mode. */
    preset_768x512_31k(regs);
    set_reg(regs, 20, 0x19);   /* HF=1, VRES=2, HRES=1 */
    CrtcTiming_FromRegs(regs, 0, &t);
    CHECK_EQ(t.scan_mode, CRTC_SCAN_INTERLACE, "31k/VRES=2: interlace scan");
    CHECK_EQ(t.v_step, 4, "31k/VRES=2: interlace mapping step");
    CHECK_EQ(t.height, 1024, "31k/VRES=2: doubled height");
}

static void test_raster_field_mapping(void)
{
    CrtcRasterMap map;

    CrtcTiming_MapRaster(CRTC_SCAN_INTERLACE, 0, 0, &map);
    CHECK(map.draw && map.line == 0, "interlace field 0 starts on even row");
    CrtcTiming_MapRaster(CRTC_SCAN_INTERLACE, 1, 0, &map);
    CHECK(map.draw && map.line == 2, "interlace field 0 advances by two");
    CrtcTiming_MapRaster(CRTC_SCAN_INTERLACE, 0, 1, &map);
    CHECK(map.draw && map.line == 1, "interlace field 1 starts on odd row");
    CrtcTiming_MapRaster(CRTC_SCAN_INTERLACE, 1, 1, &map);
    CHECK(map.draw && map.line == 3, "interlace field 1 advances by two");

    CrtcTiming_MapRaster(CRTC_SCAN_DOUBLE, 0, 0, &map);
    CHECK(!map.draw, "double-read skips first physical raster");
    CrtcTiming_MapRaster(CRTC_SCAN_DOUBLE, 1, 0, &map);
    CHECK(map.draw && map.line == 0, "double-read draws one row per pair");
}

static void test_field_parity_sequence(void)
{
    BYTE regs[48];

    CRTC_Init();
    preset_768x512_31k(regs);
    set_reg(regs, 20, 0x19);
    write_regs_to_crtc(regs);

    CHECK(CRTC_BeginField(), "interlace entry invalidates the old weave");
    CHECK_EQ(CRTC_FieldParity, 0, "first interlace field is even");
    CRTC_EndField();
    CHECK(!CRTC_BeginField(), "continuing interlace preserves the weave");
    CHECK_EQ(CRTC_FieldParity, 1, "second interlace field is odd");
    CRTC_EndField();
    CRTC_BeginField();
    CHECK_EQ(CRTC_FieldParity, 0, "third interlace field wraps to even");
    CRTC_EndField();

    set_reg(regs, 20, 0x15);  /* HF=1, VRES=1: normal */
    write_regs_to_crtc(regs);
    CHECK(CRTC_BeginField(), "leaving interlace invalidates the weave");
    CHECK_EQ(CRTC_FieldParity, 0, "normal scan has no odd parity");
    CRTC_EndField();
}
...
/* Walk all eight HF/VRES combinations and require the renderer globals and
 * pure timing model to use the same hardware scan structure. */
static void test_vertical_scan_decode_all_vres(void)
{
    static const struct {
        int hf, vres;
        int want_vstep;
        int want_height;        /* TextDotY for a 512-raster window */
        ...
        { 0, 2, 4, 1024, CRTC_SCAN_INTERLACE, "15k VRES=2" },
        { 0, 3, 4, 1024, CRTC_SCAN_INTERLACE, "15k VRES=3" },
        { 1, 0, 1,  256, CRTC_SCAN_DOUBLE,    "31k double-read" },
        { 1, 1, 2,  512, CRTC_SCAN_NORMAL,    "31k normal" },
        { 1, 2, 4, 1024, CRTC_SCAN_INTERLACE, "31k VRES=2" },
        { 1, 3, 4, 1024, CRTC_SCAN_INTERLACE, "31k VRES=3" },
    };
    size_t i;
    ...
    for (i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        snprintf(name, sizeof(name), "R20=$%02x %s: model mirrors renderer v_step",
                 r20, cases[i].note);
        CHECK_EQ(t.v_step, CRTC_VStep, name);
    }
}

/* VLINE carries the interlace parity and advances by two, so the draw
 * routines consume one VRAM row per woven-buffer row in every scan mode.
 *
...
static void test_vram_row_step(void)
{
    ...
    write_regs_to_crtc(regs);
    CHECK_EQ(latched_row_step(), 1, "31k 1024-line: parity is carried by VLINE");
    CHECK_EQ(CRTC_VStep, 4, "31k 1024-line: interlace v_step");

    preset_interlace_15k(regs);         /* HF=0, VRES=1 */
    write_regs_to_crtc(regs);
    CHECK_EQ(latched_row_step(), 1, "15k interlace: rows are sequential");
    CHECK_EQ(CRTC_VStep, 4, "15k interlace: v_step 4 selects interlace mapping");
    ...
    CHECK_EQ(latched_row_step(), 1, "latch: interlace stride remains one");
     ...
     * the stride implicitly; cached scan state must be reset explicitly. */
    CRTC_Init();
...
int main(void)
{
    ...
    test_raster_field_mapping();
    test_field_parity_sequence();
    test_hrl_and_vga();
    test_invalid_window();
