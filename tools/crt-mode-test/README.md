...
起動すると以下を無限に巡回します。各モードでグラフィックにカラーバンドを描き、
テキストプレーンにモード名を出力し続けます(テキストは消さないので、切替時の
残像・ゴミが見えれば異常です)。

**各モードはキーを押すまで表示を保持します。** 押さなければ約8秒で次に進むので、
目視で見比べたいときは好きなだけ止めておけますし、無人でASan放置検証を回すときは
そのまま巡回が続きます。
